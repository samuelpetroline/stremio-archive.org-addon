const Catalog = (dependencies) => {
  const { httpClient, queryBuilder, env } = dependencies
  ...
  const searchCatalog = async (request) => {
    const { search, genre, skip } = request
    ...
    try {
      ...
      const response = await httpClient.get(env.url.search, {
        params: {
          ...
          fields:
            'avg_rating,creator,date,description,genre,identifier,language,subject,title,type',
          q: query,
        },
      })
